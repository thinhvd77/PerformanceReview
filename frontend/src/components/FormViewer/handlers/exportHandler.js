 ...
 * Uses pendingBonusAwards data passed from bonus processor
 * 
 * @param {Object|null} pendingBonusAwards - Bonus awards data {username, year, quarter, bonusKeys}
 * @param {Object} api - API instance
 * @returns {Promise<void>}
 */
export async function recordAwardedBonuses(pendingBonusAwards, api) {
    try {
        // If no pending awards, skip
        if (!pendingBonusAwards || !pendingBonusAwards.bonusKeys || pendingBonusAwards.bonusKeys.length === 0) {
            console.log('No pending bonus awards to record');
            return;
        }

        const { username, year, quarter, bonusKeys } = pendingBonusAwards;
        console.log(`[EXPORT] Recording ${bonusKeys.length} bonus awards:`, bonusKeys);
        ...
        console.log('[EXPORT] Bonus awards recorded successfully');
    } catch (err) {
        console.warn('[EXPORT] Failed to record bonus awards:', err?.response?.data || err.message);
 ...
 * @param {number} params.scoreColIdx - Score column index
 * @param {Object} params.api - API instance
 * @param {Object|null} params.pendingBonusAwards - Bonus awards to record {username, year, quarter, bonusKeys}
...
export async function handleExportWorkflow({
    ...
    try {
        ...
        await recordAwardedBonuses(pendingBonusAwards, api);
