...
export function processGrowthRules({
    ...
    const parentRowIdx = findRowIndexByCriteria(rows, "Điểm cộng (tối đa 10 điểm)", 1, true);
    ...
    const processRule = (rule) => {
        ...
        const removeAutoRow = () => {
            ...
            const parentIdxNext = findRowIndexByCriteria(nextRows, "Điểm cộng (tối đa 10 điểm)", 1, true);
        ...
        const nextRows = rows.map((row, idx) => {
            ...
            // Find parent row by label (not STT) to support different form numbering schemes
            if (
                normalizeText(row?.cells?.[1]?.value) === normalizeText("Điểm cộng (tối đa 10 điểm)") &&
                row?.cells
            ) {
                const cells = row.cells.map((cell, cIdx) => {
                    if (cIdx === scoreColIdx) {
