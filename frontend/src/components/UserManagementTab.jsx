...
export default function UserManagementTab() {
    ...
    return (<Space direction="vertical" style={{width: '100%'}} size="large">
        ...
        <Modal
            title={<Title level={5} style={{margin: 0}}>Tạo mới người dùng</Title>}
            open={open}
            onCancel={() => {
                if (!creating) {
                    setOpen(false);
        ...
        <Card title={<Title level={5} style={{margin: 0}}>Users</Title>}>
            {users.length === 0 ? (<Text type="secondary">No users found.</Text>) : (<List
                loading={loading}
                bordered
                dataSource={users}
                renderItem={(u) => (<List.Item
                    actions={[<Button key="edit" size="small" onClick={() => {
                        setCurrentUser(u);
                        setEditOpen(true);
                    }}>Edit</Button>,
                        <Popconfirm key="del" title={`Delete user ${u.username}?`} okText="Delete"
