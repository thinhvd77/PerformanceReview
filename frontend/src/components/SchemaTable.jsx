...
export default function SchemaTable({
    ...
    const getOptionsForRow = (criteriaText) => {
        ...
        if (
            criteriaTrim &&
            Array.isArray(sectionOptions[criteriaTrim]) &&
            sectionOptions[criteriaTrim].length
        ) {
        ...
        if (
            normalized &&
            Array.isArray(sectionOptions[normalized]) &&
            sectionOptions[normalized].length
        ) {
            return sectionOptions[normalized];
        }
        const match = Object.entries(sectionOptions).find(
            ([k, opts]) => opts?.length && norm(k) === normalized
        );
                        ...
                        <tr>
                            {table.columns.map((col, idx) => {
                                ...
                                if (col.rowSpan && col.rowSpan > 1)
                                    props.rowSpan = col.rowSpan;
                                if (col.colSpan && col.colSpan > 1)
                                    props.colSpan = col.colSpan;
                                return (
                                    <th
                                        key={`h-${idx}`}
                                        {...props}
                    ...
                    <tbody>
                        {table.rows.map((row, rIdx) => {
                            ...
                            const rowKey =
                                ...
                                `row-${rIdx}`;
                            // Dòng con: có ô điểm (cột score) với addr thuộc map childAddrToParentRow
                            const scoreCell = (row?.cells || [])[scoreColIdx];
                            const childScoreAddr =
                                scoreCell && scoreCell.addr
                                    ? scoreCell.addr
                                    : null;
                            const isChildRow = !!(
                                childScoreAddr &&
                            ...
                            return (
                                <tr key={`r-${rIdx}`}>
                                    {(row.cells || []).map((cell, cIdx) => {
                                        ...
                                        // nhận diện phần trăm
                                        const percentCell = isPercentCell(
                                            cell,
                                            rawVal
                                        );
                                        ...
                                        const numericCol = isNumericColumn(
                                            table?.columns?.[cIdx]?.label
                                        );
                                        const buf = addr
                                            ? editBuf[addr]
                                            : undefined;
